...
<div class="row">
    <div class="col-md-12">
        <form method="post" action="{{ route('cms.regional_units.municipalities.update', ['regional_unit' =>$regionalUnit->id, 'municipality'=>$municipality->id]) }}">
            @csrf
            @method('put')
            <div class="form-group row">
                <label for="name" class="col-sm-2 col-form-label">Όνομα:</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control form-control-sm" id="name" name="name" value="{{ $municipality->name }}">
                </div>
            </div>
            <div class="form-group row">
                <div class="col-sm-6">
